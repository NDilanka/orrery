...
pure cores (:mod:`orrery_loop.decide`, :mod:`orrery_loop.gate`, :mod:`orrery_loop.hashlock`, …); this module is the
...
``runner`` (an :class:`~orrery_loop.runners.base.AgentRunner`), and every event is written through
:func:`orrery_loop.logio.append_event`, so the whole loop is driveable with a mock runner + a callable
...
- **No ``-AutoDecide`` / judge sub-agent calls.** The Q&A surface detects a ``QUESTION:``
  ...
  :func:`orrery_loop.verdict.parse_verdict`; ``config.verify.contract`` (or the parsed contract) is the
  frozen criteria.
...
from orrery_loop.hashlock import compare_hash_map, test_hash_map
from orrery_loop.heartbeat import Heartbeat
from orrery_loop.memory import FileMemoryStore, Lesson, NullMemoryStore
...
from orrery_loop.verdict import contract_criteria, parse_verdict, question_marker
from orrery_loop.verdict import read_answer_inbox as match_answer_inbox
...
def run_loop(
    ...
    *,
    ...
    ``runner`` is an :class:`~orrery_loop.runners.base.AgentRunner`; ``state_dir`` holds
    ...
    # --- concurrency guard (orrery_loop.driver_shell — the shared lock/checkpoint/STOP shell) -----
...
def _build_prompt(
    ...
    """Assemble the stable execute prompt via :mod:`orrery_loop.prompts`.

    ``recall`` (a cross-run lessons block from :mod:`orrery_loop.memory`) is inserted into the stable
    prefix; empty by default so the prompt is byte-identical to the pre-memory version.
    """
    from orrery_loop.prompts import execute_prompt
...
def _write_feedback_steer(
    ...
    Held-out sections are stripped FIRST via :func:`orrery_loop.verify.visible_feedback_raw` so the
    ...
    :func:`orrery_loop.feedback.compact_feedback`. Default off + no held-out stage -> the raw gate dump
...
def _resume_command(config: EngineConfig, state: Path, work: Path) -> str:
    ...
    §7). Mirrors :func:`orrery_loop.bmad.driver._resume_command`: carries the REAL task/cwd/state-dir
...
def _write_checkpoint(
    ...
    """Write a PROTOCOL §7 checkpoint.json (orrery_loop.driver_shell — the shared checkpoint shape)."""
