...
from orrery_loop.events import cooperative_stop_event
from orrery_loop.logio import append_event
from orrery_loop.resilient import ResilientRunner
from orrery_loop.runners.claude import ClaudeRunner
...
class QaConfig:
    ...
    manifest_path: str  # ac-manifest.json (built by orrery_loop.qa.manifest)
    ...
    def from_loop_json(data: dict, *, project_root: str | None = None) -> "QaConfig":
        ...
        accepted (:func:`orrery_loop.configkeys.resolve`); an unrecognized key warns on stderr
        (:func:`orrery_loop.configkeys.warn_unknown_keys`).
        """
...
def default_invoke(
    ...
    *,
    ...
    Goes through the shared :class:`~orrery_loop.runners.claude.ClaudeRunner` (the ONE place a real
    ``claude`` process spawns) wrapped in :class:`~orrery_loop.resilient.ResilientRunner`, so an overnight
...
def run(
    ...
    *,
    ...
    Acquires the SAME single-flight lock (:mod:`orrery_loop.lockfile`, via :mod:`orrery_loop.driver_shell`)
...
def _run_inner(
    ...
    *,
    ...
    for i, epic in enumerate(epics, start=1):
        ...
        # mode (phase/story/now/empty) is honored — see orrery_loop.checkpoint.get_stop_mode.
