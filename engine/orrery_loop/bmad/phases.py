...
- a :class:`orrery_loop.runners.base.AgentRunner` (``runner``) — the ONLY thing that spawns a real
  ``claude`` process;
...
This module also provides a REAL :class:`DevServer` (uses :mod:`orrery_loop.proc`) that the driver
...
from orrery_loop.bmad.story import story_acs
...
def code_review(
    ...
    *,
    ...
    (detected by :func:`orrery_loop.verdict.question_marker`), bounded by ``max_turns``:
...
def browser_smoke(
    ...
    *,
    ...
    then build the AC-aware prompt from :func:`orrery_loop.bmad.story.story_acs` of ``story_text`` and
...
# ---------------------------------------------------------------------------
# REAL DevServer (production server_ctl) — uses orrery_loop.proc; NOT in the unit tests
# ---------------------------------------------------------------------------
...
class DevServer:
    ...
    ``stop()`` tree-kills the spawned process (``orrery_loop.proc.kill_tree``) and reaps stray
    dev-server / chrome processes by name (``orrery_loop.proc.kill_by_name``), SCOPED to ``cwd`` so
    ...
    def stop(self) -> None:
        ...
        ``orrery_loop.proc.kill_by_name(within_dir=...)``.
        """
