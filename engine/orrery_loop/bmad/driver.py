...
:class:`orrery_loop.bmad.phases.DevServer`); ``gh`` PR/merge go through :mod:`orrery_loop.bmad.pr` (which
tests monkeypatch); git is read/written via :mod:`orrery_loop.gitutil`; events go through ``emit``
...
- Concurrency guard is a pid lockfile (like :func:`orrery_loop.core.run_loop`), not a Win32 process
  scan.
...
from orrery_loop.decide import floor_breach
from orrery_loop.driver_shell import read_stop_request, run_driver, write_checkpoint_now
from orrery_loop.bmad import phases, pr, recovery
...
from orrery_loop.resilient import ResilientRunner, _usage_tokens  # re-exported for back-compat  # noqa: F401
from orrery_loop.runners.base import AgentRunner
from orrery_loop.verdict import question_marker
...
# (orrery_loop.configkeys.warn_unknown_keys) instead of silently vanishing.
_BMAD_KNOWN_KEYS = {
...
class BmadConfig:
    ...
    def from_loop_json(cls, path_or_dict: Any) -> BmadConfig:
        ...
        Every field accepts EITHER camelCase or snake_case (:func:`orrery_loop.configkeys.resolve`);
        an unrecognized key in the block warns on stderr instead of silently vanishing
        (:func:`orrery_loop.configkeys.warn_unknown_keys`). ``gate_stages``/``gateStages`` (previously
...
def run(config: BmadConfig, *, runner: AgentRunner, state_dir, cwd=None) -> int:
    ...
    # ONE lockfile name ("lock") shared with orrery_loop.core / orrery_loop.qa.discover (orrery_loop.lockfile) — a
...
def _gate_checkpoint(
    ...
    *,
    ...
    below the running ``floor`` (:func:`orrery_loop.decide.floor_breach`) HALTS as a regression; otherwise
...
# up to ~(2N+1) cold ones. Mirrors :func:`orrery_loop.bmad.phases.code_review_single_pass`.
