...
- :func:`main` (``loop``)      — run the generic fix-until-green loop (port of ``loop.ps1``'s
  param surface, reduced to the OSS essentials). Builds an :class:`~orrery_loop.config.EngineConfig`
  (from ``--loop-json`` if given, else from flags/defaults; CLI flags override), gets a runner
  via :func:`~orrery_loop.runners.get_runner`, and calls :func:`~orrery_loop.core.run_loop`.
...
- :func:`main_bmad` (``loop-bmad``) — the BMAD multi-story driver entrypoint: build a
  :class:`~orrery_loop.bmad.driver.BmadConfig` from the CLI flags, get a base runner, and run the
  full epic pipeline (:func:`orrery_loop.bmad.driver.run`).
"""
...
from orrery_loop.config import EngineConfig, from_loop_json
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--compact-feedback",
        action="store_true",
        help="feed back only the first failing test (orrery_loop.feedback) instead of the raw gate dump",
    )
    parser.add_argument(
        "--memory",
        nargs="?",
        ...
        metavar="PATH",
        help="enable the cross-run lessons store (orrery_loop.memory); optional PATH to the JSONL file",
    )
    parser.add_argument(
        "--mutation-audit",
        action="store_true",
        help="run the advisory mutation-strength audit when the gate is green (orrery_loop.verify)",
    )
    parser.add_argument(
        "--emit-metrics",
        action="store_true",
        help="emit a run-quality metrics event at stop (orrery_loop.metrics)",
    )
...
def main_bmad(argv: list[str] | None = None) -> int:
    ...
    :class:`~orrery_loop.bmad.driver.BmadConfig` from the flags, gets the BASE runner via
    :func:`~orrery_loop.runners.get_runner` (the driver wraps it in a quota-surviving
    ``ResilientRunner``), and dispatches to :func:`orrery_loop.bmad.driver.run`. ``--project-root`` is
...
def main_qa(argv: list[str] | None = None) -> int:
    ...
    :mod:`orrery_loop.qa.manifest`) are REQUIRED; an optional ``--loop-json`` ``qa`` block supplies tuning
...
def main_supervise(argv: list[str] | None = None) -> int:
    ...
    from the wrapped command ends supervision cleanly. See :mod:`orrery_loop.supervise`.
    """
    from orrery_loop.supervise import SupervisorConfig, supervise
