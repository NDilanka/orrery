...
Kept here (not inline in :mod:`loop.core`) so the prompt text stays readable and so the
...
def execute_prompt(
    ...
    ``recall`` is an OPTIONAL cross-run lessons block (:mod:`loop.memory`); when non-empty it is
