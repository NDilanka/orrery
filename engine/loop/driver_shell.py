...
``loop.core.run_loop``, ``loop.bmad.driver.run``, and ``loop.qa.discover.run`` each hand-wired
...
at (:func:`loop.checkpoint.get_stop_mode`), and release the lock on every exit path. This module
...
1. **State files live under ``state_dir``** (PROTOCOL §1): ``log.jsonl`` (append-only —
   :func:`loop.logio.append_event`), ``checkpoint.json`` (:func:`write_checkpoint_now`),
   ``STOP`` (cooperative-stop flag), and ``activity.json`` (the liveness heartbeat —
   :class:`loop.heartbeat.Heartbeat`, wrapped around every blocking agent call). A driver may
...
2. **The lock is exclusive and automatic.** ``run_driver`` acquires
   ``<state_dir>/lock`` (:mod:`loop.lockfile` — the ONE lock name shared by every driver, so a
...
4. **Checkpoints follow ONE shape.** Every checkpoint write goes through
   :func:`write_checkpoint_now` (which wraps :func:`loop.events.new_checkpoint` +
   :func:`loop.logio.write_checkpoint`) so ``updatedAt``/``resume``/``cumUsd`` rounding are
...
from loop.events import new_checkpoint
from loop.lockfile import LOCK_NAME, acquire_lock, release_lock
from loop.logio import read_stop_flag, write_checkpoint
...
def run_driver(
    ...
    *,
    ...
    (:mod:`loop.lockfile`), calls ``body(state)`` with the lock held, and releases the lock in a
...
def read_stop_request(stop_path: Any, scope: str) -> dict[str, Any]:
    ...
    A thin pairing of :func:`loop.logio.read_stop_flag` (the file read) with
    :func:`loop.checkpoint.get_stop_mode` (the pure normalize-and-decide) — every driver did this
    exact pairing inline; this is the one place it lives now. Returns
    ``{requested, honor, mode}`` (see :func:`loop.checkpoint.get_stop_mode`).
    """
...
def write_checkpoint_now(
    ...
    *,
    ...
    Wraps :func:`loop.events.new_checkpoint` (the pure builder — stamps ``updatedAt``, rounds
    ``cumUsd``) and :func:`loop.logio.write_checkpoint` (the file write) so no driver hand-rolls
