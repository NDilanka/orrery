...
either camelCase or snake_case" fallback: :mod:`loop.config` had ``_first``, ``BmadConfig
...
generalization of the original ``loop.config._first``, moved here so it isn't private to one
module.
    ...
    *,
    ...
    for key in block:
        ...
        if key in retired_set:
            print(
                f"[loop.config] '{where}': '{key}' is retired and no longer has any effect.",
                file=sys.stderr,
            )
        else:
            print(
                f"[loop.config] '{where}': unrecognized key '{key}' (typo? it is ignored).",
                file=sys.stderr,
            )
