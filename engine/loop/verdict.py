...
data. The wire shape of a parsed verdict is built by :func:`loop.events.verdict_event` so
...
from loop.events import verdict_event
...
def parse_verdict(raw_text: str | None, item: str | None, model: str | None) -> dict[str, Any]:
    ...
    ``verdict`` event object (built via :func:`loop.events.verdict_event`). The judge body
