...
runner, runners injected by the caller). They layer ON TOP of :mod:`loop.gate`
(``run_gate`` produces the ``gate_result`` consumed here) and :mod:`loop.hashlock`
...
def held_out_lock_globs(stages: list[dict[str, Any]] | None) -> list[str]:
    ...
    files cannot be edited by the agent (see :mod:`loop.hashlock`). A stage may give
