...
compact signal back (into the volatile steer that :mod:`loop.prompts` keeps OUT of the cached
...
- :func:`compact_feedback` — convenience over a :func:`loop.gate.run_gate` result, which
...
# (keeps it a leaf module; the byte pattern is identical to ``loop.gate.strip_ansi``).
# ---------------------------------------------------------------------------------------
...
def compact_feedback(gate_result: dict, *, max_chars: int = 1200) -> str:
    """Compact the FIRST failing stage of a :func:`loop.gate.run_gate` result, or ``""``.
